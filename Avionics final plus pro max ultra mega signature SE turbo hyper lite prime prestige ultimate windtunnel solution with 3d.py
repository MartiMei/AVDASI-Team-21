...
def cali_port(angle):
    return int(5.7021 * angle + 1500)

def cali_starboard(angle):
    return int(0.0561 * angle ** 2 + 12.817 * angle + 1400)
...
def set_servo_angle():
    ...
    if emergency==0:
        ...
        if selected_wing == 1:
            pwm_aileron = cali_port(angle_aileron)
        else:
            pwm_aileron = cali_starboard(angle_aileron)
        ...
        #pwm_elevator = int(0.0594*(angle_elevator)**2-11.167*(angle_elevator)+1500)
        pwm_elevator = int(-9.0167*(angle_elevator)+1500)
        if pwm_elevator < 1200:
            pwm_elevator = 1092
        ...
        pwm_rudder = int(0.0586*angle_rudder**2-10.845*angle_rudder + 1510)
...
def mavlink_thread():
    ...
    global last_flap_pwm, sensor_data,flap_switch_value,pot_voltage,servo1_pwm, servo2_pwm, servo4_pwm,armed_state,safety_enabled,override_active, selected_wing
    ...
    while not exit_program:
        ...
        # --- Recieving messages of the current PWM value of aileron/elevator servo ---
            ...
            #Use pwm cause angles were not accurate because the mechcanical design unstable
            aileron_text.set(f"Aileron pwm: {servo1_pwm:.1f}°")
            elevator_text.set(f"Elevator pwm: {servo2_pwm:.1f}°")
            #elevator_text.set(f"Elevator angle: {elevator_angle:.1f}°")
            rudder_text.set(f"Rudder pwm: {servo4_pwm:.1f}°")
        ...
        # --- Recieving the current volatge of the potentiometer ---
            ...
            if name == "POT_VOLT":
                ...
                #Display the angle of flap ---- angle not stable because of manufacture
                #Display voltage
                voltage_text.set(f"Flap voltage: {pot_voltage:.2f}V")
            ...
            if python_control:
                ...
                if emergency==0:
                    is_port = selected_wing == 1
                    if flap_value == 0:
                        flap_target_voltage=0
                    elif flap_value == 1:
                        #CHNAGE THESE VOLTAGE TARGETS, CALIBRATED TO WING
                        flap_mode = "UP"
                        flap_target_voltage = 1.48 if is_port else 2.40
                    elif flap_value == 2:
                        flap_mode = "TAKEOFF"
                        flap_target_voltage = 1.24 if is_port else 2.50
                    elif flap_value == 3:
                        flap_mode = "LANDING"
                        flap_target_voltage = 1.01 if is_port else 2.60
                    elif flap_value==4:
                        ...
                        if is_port:
                            ...
                            flap_target_voltage = round(0.0067 * entered_flap_angle + 2.40, 2)
...
def on_wing_change():
    global selected_wing
    selected_wing = wing.get()
    cmd_logging("WING_SELECTION", wing="PORT" if selected_wing == 1 else "STARBOARD")

Radiobutton(wing_frame, variable=wing, value=1, text="Port", command=on_wing_change).grid(row=1, column=0)
Radiobutton(wing_frame, variable=wing, value=2, text="Starboard", command=on_wing_change).grid(row=1, column=1)
# -----------------------------
# Emergency mode
# -----------------------------
...
def update_3d_model():
    ...
    # Body
    body = np.array([
        [-1,-body_r,-body_r],[ 1,-body_r,-body_r],
        [ 1, body_r,-body_r],[-1, body_r,-body_r],
        [-1,-body_r, body_r],[ 1,-body_r, body_r],
        [ 1, body_r, body_r],[-1, body_r, body_r],
    ]).T
    ...
    # Wing
    wings = np.array([
        [wing_x,-wing_span,0],[wing_x-wing_chord,-wing_span,0],
        [wing_x-wing_chord, wing_span,0],[wing_x, wing_span,0]
    ]).T
    plane_parts['wing'].set_verts([list(zip(wings[0],wings[1],wings[2]))])
    ...
    flap_L = np.array([
        [hinge,-flap_start,0],[wing_x-flap_depth,-flap_start,0],
        [wing_x-flap_depth, flap_start,0],[hinge, flap_start,0]
    ]).T
    ...
    ail_L = np.array([
        [hinge,-wing_span,0],[wing_x-aileron_depth,-wing_span,0],
        [wing_x-aileron_depth,-ail_start,0],[hinge,-ail_start,0]
    ]).T
    ail_L = rotate_surface(ail_L, aileron_angle, hinge, 'y')
    plane_parts['aileron_L'].set_verts([[ail_L[:,0],ail_L[:,1],ail_L[:,2],ail_L[:,3]]])

    ail_R = np.array([
        [hinge, ail_start,0],[wing_x-aileron_depth, ail_start,0],
        [wing_x-aileron_depth, wing_span,0],[hinge, wing_span,0]
    ]).T
    ...
    # Tail
    tail = np.array([
        [tail_x,-tail_span/2,0],[tail_x+tail_chord,-tail_span/2,0],
        [tail_x+tail_chord, tail_span/2,0],[tail_x, tail_span/2,0]
    ]).T
    plane_parts['tail_fixed'].set_verts([[tail[:,0],tail[:,1],tail[:,2],tail[:,3]]])

    hinge_t = tail_x + tail_chord * 0.6
    elev = np.array([
        [hinge_t,-tail_span/2,0],[tail_x+tail_chord,-tail_span/2,0],
        [tail_x+tail_chord, tail_span/2,0],[hinge_t, tail_span/2,0]
    ]).T
    ...
    # Fin
    fin = np.array([
        [tail_x,0,0],[tail_x+fin_chord,0,0],
        [tail_x+fin_chord,0,fin_height],[tail_x,0,fin_height]
    ]).T
    plane_parts['fin_fixed'].set_verts([[fin[:,0],fin[:,1],fin[:,2],fin[:,3]]])

    hinge_r = tail_x + fin_chord * 0.6
    rud = np.array([
        [hinge_r,0,0],[tail_x+fin_chord,0,0],
        [tail_x+fin_chord,0,fin_height],[hinge_r,0,fin_height]
    ]).T
