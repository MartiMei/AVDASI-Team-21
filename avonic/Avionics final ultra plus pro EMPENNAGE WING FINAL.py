...
def mavlink_thread():
    ...
    SERVO_STEP = 2               # PWM increment of flap servo per loop (small increments)
            ...
            if python_control:
                ...
                if emergency==0:
                    if flap_value == 0:
                        ...
                        flap_mode = "UP"
                        flap_target_voltage = 2.4
                    elif flap_value == 2:
                        flap_mode = "TAKEOFF"
                        flap_target_voltage = 2.5
                    elif flap_value == 3:
                        flap_mode = "LANDING"
                        flap_target_voltage = 2.6
                    elif flap_value==4:
                        #CREATE EQUATION FOR FLAP DEFLCTION
                        flap_mode = "CUSTOM"
                        entered_flap_angle = float(flap_angle_entry.get())
                        flap_target_voltage = round(-0.075*entered_flap_angle + 2.39,2)
...
Button(angle_frame, text="Test", command=lambda: servo_test(1, 950, 2050)).grid(row=0, column=2, padx=10, pady=5, sticky="n") 
...
Button(angle_frame, text="Test", command=lambda: servo_test(2, 950, 2050)).grid(row=1, column=2, padx=10, pady=5)
...
Button(angle_frame, text="Test", command=lambda: servo_test(4, 950, 2050)).grid(row=2, column=2, padx=10, pady=5)
