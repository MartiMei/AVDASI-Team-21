...
csv_writer.writerow(["Time", "Yaw", "Roll", "Pitch","Pitch rate","Servo 1 PWM","Servo 2 PWM","Servo 4 PWM"])
...
def set_servo_angle():
    ...
    aileron_pwm=pwm_aileron,
    elevator_deg=angle_elevator,
    elevator_pwm=pwm_elevator,
    rudder_deg=angle_rudder,
    rudder_pwm=pwm_rudder) #log angle change
...
def mavlink_thread():
    ...
    SERVO_STEP = 10                # PWM increment per loop (small increments)
    ...
    while not exit_program: 
        ...
        if recording:
            ...
            if not python_control:
                csv_writer.writerow([timestamp, round(yaw, 1), round(roll, 1), round(pitch, 1),round(pitch_rate,1),servo1_pwm,servo2_pwm,servo4_pwm])
            else:
                csv_writer.writerow([timestamp, round(yaw, 1), round(roll, 1), round(pitch, 1),round(pitch_rate,1),aileron.get(),elevator.get(),rudder.get()])
            """
            csv_writer.writerow([timestamp, round(yaw, 1), round(roll, 1), round(pitch, 1),round(pitch_rate,1),servo1_pwm,servo2_pwm,servo4_pwm])
            csv_file.flush()
        ...
        if msg.get_type() == "RC_CHANNELS":
            ...
            if flap_switch_value < 1400:
                ...
                flap_target_voltage = 2.0
            elif flap_switch_value < 1550:
                flap_mode = "TAKEOFF"
                flap_target_voltage = 2.5
            else:
                flap_mode = "LANDING"
                flap_target_voltage = 3.0
            ...
            if flap_mode != last_flap_mode:  #flap logging
                ...
                rc_value=flap_switch_value)
            last_flap_mode = flap_mode
            flap_ontarget = False #True when reached PWM
            ...
            if not python_control:
                ...
                if flap_target_voltage is None: #redundancy code
                    continue #skip this process, when using return, it would jump out mavlink_trhread

                if abs(pot_voltage - flap_target_voltage) <= VOLTAGE_TOLERANCE:
                    if not flap_ontarget:
                        flap_ontarget = True
                        cmd_logging(
                        "FLAP_REACHED",
                        mode=flap_mode,
                        voltage=round(pot_voltage, 3),
                        pwm=flap_pwm)
                else:
                    flap_ontarget = False
                    if pot_voltage < flap_target_voltage:
                        flap_pwm += SERVO_STEP
                    else:
                         flap_pwm -= SERVO_STEP
...
canvas3 = FigureCanvasTkAgg(fig3, master=plot_frames)
...
fig_airspeed, ax_airspeed = plt.subplots(figsize=(4, 2))
...
ax_airspeed.legend(loc="upper right")

canvas_airspeed = FigureCanvasTkAgg(fig_airspeed, master=plot_frames)
canvas_airspeed.get_tk_widget().grid(row=2,column=0)

# -----------------------------
# Servo pwm live plot
# -----------------------------

servo_fig,servo_ax = plt.subplots(figsize=(4, 2))
...
servo_canvas.get_tk_widget().grid(row=3,column=0)
...
angle_frame.grid(row=3, column=1, padx=10, pady=10, sticky="nw") 
...
slider_frame.grid(row=4,column=1)
...
log_frame.grid(row=5, column=1, columnspan=3,padx=10,pady=10,sticky="nsew")
...
# Buffers for control vs attitude correlation
elevator_vals = deque(maxlen=300)
pitch_vals = deque(maxlen=300)

aileron_vals = deque(maxlen=300)
roll_vals = deque(maxlen=300)

rudder_vals = deque(maxlen=300)
yaw_vals = deque(maxlen=300)
...
control_mode_text = StringVar(value="Mode: RC Control")
Label(root, textvariable=status_text, font=("Arial", 12)).grid(row=8, column=0, padx=10, pady=10)
Label(root, textvariable=attitude_text, font=("Arial", 11)).grid(row=1, column=4, padx=10, pady=10)

Label(root, text="Manual control", font="calibri 16 bold italic underline").grid(row=0, column=0, padx=10, pady=10)

rudder_value = StringVar(value="PWM: 1495")
elevator_value = StringVar(value="PWM: 1495")
aileron_value = StringVar(value="PWM: 1495")

rudder = tk.Scale(root, from_=900, to=2000, orient="horizontal", command=update_rudder, length=200, showvalue=False)
rudder.set(1450)
rudder.grid(row=3, column=0, padx=10, pady=10)
Label(root, textvariable=rudder_value).grid(row=3, column=0, sticky="w", padx=5)

elevator = tk.Scale(root, from_=900, to=2000, orient="vertical", command=update_elevator, length=200, showvalue=False)
elevator.set(1450)
elevator.grid(row=2, column=1, padx=10, pady=10)
Label(root, textvariable=elevator_value).grid(row=2, column=2, sticky="w", padx=5)

aileron = tk.Scale(root, from_=900, to=2000, orient="horizontal", command=update_aileron, length=200, showvalue=False)
aileron.set(1450)
aileron.grid(row=3, column=1, padx=10, pady=10)
Label(root, textvariable=aileron_value).grid(row=3, column=2, sticky="w", padx=5)

recentre = Button(root, text="Re-centre controls", command=centre_sliders)
recentre.grid(row=4, column=1, padx=10, pady=10)

toggle_button = Button(root, text="Start Logging", command=toggle_recording,bg="green")
toggle_button.grid(row=4, column=0, padx=10, pady=10)

quit_button = Button(root, text="Quit", command=quit_program,bg="red")
quit_button.grid(row=6, column=0, padx=10, pady=10, columnspan=2)

Label(root, textvariable=control_mode_text, font=("Arial", 12)).grid(row=2, column=0, padx=10, pady=10)

#Servo test
#lambda for delay call, according to internet
Button(root, text="Rudder Max/Min Test", command=lambda: servo_test(4, 1100, 1900)).grid(row=8, column=4, padx=10, pady=5)

Button(root, text="Elevator Max/Min Test", command=lambda: servo_test(2, 1100, 1900)).grid(row=9, column=4, padx=10, pady=5)
#have no idea why the aileron button has significantly big gap between former ones when without sticky="n"
Button(root, text="Aileron Max/Min Test", command=lambda: servo_test(1, 1100, 1900)).grid(row=10, column=4, padx=10, pady=5, sticky="n") 

#Angle input set_servo_angle
angle_frame = Frame(root)
angle_frame.grid(row=10, column=3, padx=10, pady=10, sticky="nw") 
...
# Set angle button
angle_set_button = Button(angle_frame, text="Set Angles", command=set_servo_angle)
angle_set_button.grid(row=3, column=0, columnspan=2, pady=10)
...
log_frame.grid(row=11, column=4, columnspan=5,padx=10,pady=10,sticky="nsew")
Label(log_frame, text="Command Logging").pack(anchor="w")
...
log_text.config(yscrollcommand=log_scroll.set) #combine text and scrolbar
# -----------------------------
# Control Response Plots (Right side) - Live Moving Points
# -----------------------------
fig2, axes = plt.subplots(3, 1, figsize=(4, 6))
fig2.subplots_adjust(left=0.2, right=0.95, top=0.95, bottom=0.1, hspace=0.5)
...
canvas3.get_tk_widget().grid(row=2, column=4, rowspan=8, padx=20, pady=10)



# Elevator vs Pitch
axes[0].set_title("Elevator vs Pitch")
axes[0].set_xlabel("Elevator PWM (µs)")
axes[0].set_ylabel("Pitch (°)")
axes[0].set_xlim(900, 2000)
axes[0].set_ylim(-90, 90)
axes[0].grid(True)
point_elev, = axes[0].plot([], [], "o", color="lime", markersize=8)

# Aileron vs Roll
axes[1].set_title("Aileron vs Roll")
axes[1].set_xlabel("Aileron PWM (µs)")
axes[1].set_ylabel("Roll (°)")
axes[1].set_xlim(900, 2000)
axes[1].set_ylim(-90, 90)
axes[1].grid(True)
point_ail, = axes[1].plot([], [], "o", color="dodgerblue", markersize=8)

# Rudder vs Yaw
axes[2].set_title("Rudder vs Yaw")
axes[2].set_xlabel("Rudder PWM (µs)")
axes[2].set_ylabel("Yaw (°)")
axes[2].set_xlim(900, 2000)
axes[2].set_ylim(-90, 90)
axes[2].grid(True)
point_rud, = axes[2].plot([], [], "o", color="red", markersize=8)

fig2.tight_layout()

canvas2 = FigureCanvasTkAgg(fig2, master=root)
canvas2.get_tk_widget().grid(row=2, column=3, rowspan=8, padx=20, pady=10)
...
canvas.get_tk_widget().grid(row=10, column=0, columnspan=2, padx=10, pady=10)
...
canvas_airspeed = FigureCanvasTkAgg(fig_airspeed, master=root)
canvas_airspeed.get_tk_widget().grid(row=2, column=8, columnspan=2, padx=10, pady=10)
...
servo_canvas = FigureCanvasTkAgg(servo_fig, master=root)
servo_canvas.get_tk_widget().grid(row=3, column=8, columnspan=2, padx=10, pady=10)
"""
...
def update_plot():
    ...
    if len(airspeed_data) > 2:
        ...
        ax_airspeed.set_xlim(max(0, airspeed_time[0]), airspeed_time[-1])
        canvas_airspeed.draw_idle()

    if len(servo1_data) > 2:
        servo1_line.set_data(list(servo_time), list(servo1_data))
        servo2_line.set_data(list(servo_time), list(servo2_data))
        servo4_line.set_data(list(servo_time), list(servo4_data))
        servo_ax.set_xlim(max(0, servo_time[0]), servo_time[-1])
