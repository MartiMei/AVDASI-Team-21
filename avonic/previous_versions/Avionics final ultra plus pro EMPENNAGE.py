...
def enable_pitch_target():
    global pitch_target_override
    pitch_target_override=1
    
def disable_pitch_target():
    global pitch_target_override
    pitch_target_override=0

disable_pitch_target()
...
def set_pitch_angle():
    return (float(pitch_target_entry.get()))
...
def mavlink_thread():
    ...
    while not exit_program:
        ...
        if msg.get_type() == "ATTITUDE":
            ...
            if pitch_target_override==1:
                if emergency==0:
                    target_flap_angle=set_pitch_angle()
                    
                    if pitch < target_flap_angle - ANGLE_TOLERANCE:
                        elevator_pwm2+=SERVO_STEP
                    elif pitch > target_flap_angle + ANGLE_TOLERANCE:
                        elevator_pwm2-=SERVO_STEP
                    
                    elevator_pwm2 = max(950, min(2050, elevator_pwm2))

                    elevator.set(elevator_pwm2)                
...
pitch_frame = Frame(info_frame)
pitch_frame.grid(row=4, column=2,columnspan=2,padx=10,pady=10,sticky="w")

Label(pitch_frame, text="Pitch target", font="calibri 16 bold italic underline").grid(row=0, column=0, padx=10, pady=10)

pitch_target_entry=Entry(pitch_frame,width=10)
pitch_target_entry.grid(row=1,column=0,padx=5, pady=5)
pitch_target_entry.insert(0, "0")

set_pitch = ctk.CTkButton(pitch_frame,command=set_pitch_angle,text="SET PITCH ANGLE TARGET",fg_color="grey",text_color="black")
set_pitch.grid(row=1, column=1, padx=10, pady=10)

activate_pitch_angle=ctk.CTkButton(pitch_frame,command=enable_pitch_target,text="Activate",fg_color="grey",text_color="black")
activate_pitch_angle.grid(row=2, column=0, padx=10, pady=10)

deactivate_pitch_angle=ctk.CTkButton(pitch_frame,command=disable_pitch_target,text="De-activate",fg_color="grey",text_color="black")
deactivate_pitch_angle.grid(row=2, column=1, padx=10, pady=10)

pitch_status=StringVar(value="")
pitch_status_label=Label(pitch_frame,textvariable=pitch_status,font=("Arial", 18),fg="red")
pitch_status_label.grid(row=2, column=2, padx=10, pady=10)
