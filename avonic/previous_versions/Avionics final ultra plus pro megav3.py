...
def set_servo_angle():
    ...
    angle_elevator_adjusted = int(-0.5687*angle_elevator)
    pwm_elevator = int(center_pwm + (angle_elevator / 40) * (2500 - center_pwm))
...
def mavlink_thread():
    ...
    flap_pwm = load_flap_pwm()   # loading the previous PWM of the flap servo, to prevent a vibrations of flap
    ...
    while not exit_program:
            ...
            if new_python_control != python_control:
                ...
                if python_control:
                    ...
                    master.mav.rc_channels_override_send(
                        master.target_system,
                        master.target_component,
                        1495, 1417, 1495, 1495, 1489, 0, 0, 0
                    )
        ...
        # --- Recieving messages of the current PWM value of aileron/elevator servo ---
            ...
            elevator_angle=(((((servo2_pwm+(1500-1417))-1495)/(2500-1495))*40)/-0.5687)
        ...
        if python_control and not safety_enabled:
            master.mav.rc_channels_override_send(
                ...
                master.target_component,
                int(aileron.get()),
                (int(elevator.get())-(1500-1417)),#1417
                int(aileron.get()),
                int(rudder.get()),
                ((1500-int(elevator.get()))+1500)-(1500-1489), 0, 0, 0#1489
            )
