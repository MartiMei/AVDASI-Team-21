...
# -----------------------------
# Angle input control(main servo)
# -----------------------------
center_pwm = 1495
...
def set_servo_angle():
    ...
    if not python_control:
        tk.messagebox.showwarning(
            "Mode Error",
            "Switch to PYTHON control before setting flap angle."
        )
        return
    ...
    if safety_enabled:
        tk.messagebox.showwarning(
            "Safety ON",
            "Disable safety before moving flap."
        )
        return

    angle_flap = max(0, min(30, angle_flap))
    pwm_flap = int(center_pwm + (angle_flap / 30) * (2000 - center_pwm))

    set_flap_servo(pwm_flap)
...
# -----------------------------
# Angle input control(aux servo)
# -----------------------------

flap_servo_number = 11   # AUX OUT 3

def set_flap_servo(pwm):
    global flap_pwm, last_flap_pwm

    pwm = max(1000, min(2000, int(pwm)))

    try:
        angle_flap = float(flap_angle_entry.get())
    except ValueError:
        tk.messagebox.showerror("Invalid Input!")
        return
...
servo_aux3_data = deque(maxlen=300)
servo_time = deque(maxlen=300)



def mavlink_thread():
    global recording, exit_program, python_control, yaw_offset,airspeed,last_flap_pwm, sensor_data,flap_switch_value,pot_voltage,servo1_pwm, servo2_pwm, servo4_pwm,armed_state,safety_enabled,override_active
    ...
    while not exit_program: 
        ...
        if recording:
            ...
            servo_aux3_data.append(servo_aux3_pwm)
...
"""
toggle_button = Button(info_frame, text="Show Plots", command=toggle_recording,bg="green")
...
angle_frame.grid(row=0, column=1, padx=10, pady=10, sticky="s") 
...
# Flap
Label(angle_frame, text="Flap (°):").grid(row=3, column=0, padx=5, pady=5, sticky="e")
flap_angle_entry = Entry(angle_frame, width=5)
flap_angle_entry.grid(row=3, column=1, padx=5, pady=5)
flap_angle_entry.insert(0, "0")
Button(angle_frame, text="Test", command=lambda: servo_test(11, 1100, 1900)).grid(row=3, column=2, padx=10, pady=5)
